...
- 60e7d3e: Only conform an absorbing `anyOf` object to its members' interfaces when it can satisfy them.
...
- 44a1833: Derive a wrapper's value property and a `$ref`'d parameter's native type from the usage they end
  up with.
...
- 89746d0: Re-derive the native type of properties that absorbing makes optional.
...
- 3f5aa8d: Support OpenAPI 3.1 nullable idiom in `oneOf`
