import { CodegenExample, CodegenExamples, CodegenSchemaLike, CodegenSchemaType, CodegenTypeInfo } from '@openapi-generator-plus/types'
...
function exampleValue(value: any, mediaType: string | undefined, schema: CodegenSchemaLike, state: InternalCodegenState): Pick<CodegenExample, 'value' | 'valueLiteral' | 'valueString' | 'valuePretty'> {
	return {
		value,
...
export function toCodegenExample(example: any, mediaType: string | undefined, schema: CodegenSchemaLike, state: InternalCodegenState): CodegenExample {
	return {
		name: null,
		summary: null,
		description: null,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function toCodegenExamples(example: any | undefined, examples: OpenAPIV2.ExampleObject | OpenAPIV3Examples | undefined, mediaType: string | undefined, schema: CodegenSchemaLike, state: InternalCodegenState): CodegenExamples | null {
	if (example) {
		return idx.create([
